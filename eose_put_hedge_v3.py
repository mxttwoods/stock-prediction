...
CURRENT_POSITIONS = [
    {"label": "7d 11P", "strike": 11.0, "days_to_exp": 7,  "premium": 0.16, "contracts": 3},
    {"label": "14d 9P", "strike": 9.0,  "days_to_exp": 14, "premium": 0.08, "contracts": 6},
    {"label": "23d 8P", "strike": 8.0,  "days_to_exp": 23, "premium": 0.15, "contracts": 5},
]
...
print(f"\n{'='*80}")
print(f"🚀 EOSE HEDGE ANALYZER - AI-POWERED OPTIONS RECOMMENDATION ENGINE")
print(f"{'='*80}\n")
...
# Current BB values
current_sma = df["SMA20"].iloc[-1] if not pd.isna(df["SMA20"].iloc[-1]) else current_price
current_std = close.rolling(BB_WINDOW).std().iloc[-1] if not pd.isna(close.rolling(BB_WINDOW).std().iloc[-1]) else daily_vol * current_price
...
def project_bollinger_bands_forward(current_price, sma, std, days_ahead, daily_vol):
    ...
    current_std_val = std.iloc[-1] if not pd.isna(std.iloc[-1]) else daily_vol * current_price
    ...
    return future_dates, np.array(projected_sma), np.array(projected_bb_up), np.array(projected_bb_dn)

future_dates, proj_sma, proj_bb_up, proj_bb_dn = project_bollinger_bands_forward(
    current_price, df["SMA20"], close.rolling(BB_WINDOW).std(),
    FORWARD_PROJECTION_DAYS, daily_vol
)
...
def calculate_price_probability(target_price: float, current_price: float,
                                days: int, daily_vol: float, annual_vol: float) -> float:
...
def analyze_put_option(put_row: pd.Series, current_price: float, shares_held: int,
                       daily_vol: float, annual_vol: float) -> Dict:
    """
    ...
    premium = float(put_row['lastPrice']) if not pd.isna(put_row['lastPrice']) else (float(put_row['bid']) + float(put_row['ask'])) / 2
    dte = int(put_row['DTE'])
    exp_date = put_row['exp_date']

    # Calculate metrics
    prob_itm = calculate_price_probability(strike, current_price, dte, daily_vol, annual_vol)
    ...
    return {
        ...
        'open_interest': int(put_row['openInterest']) if 'openInterest' in put_row else 0,
    }


def recommend_optimal_hedge(options_df: pd.DataFrame, current_price: float,
                           shares_held: int, budget: float, daily_vol: float,
                           annual_vol: float, expected_low: float) -> List[Dict]:
    """
    ...
    for idx, row in options_df.iterrows():
        try:
            analysis = analyze_put_option(row, current_price, shares_held, daily_vol, annual_vol)
    ...
    for exp_date in protective_puts['exp_date'].unique()[:4]:  # Top 4 expirations
        ...
        if cost_per_contract > 0 and remaining_budget >= cost_per_contract:
            contracts = min(int(remaining_budget / cost_per_contract),
                           int(shares_held / 100))  # Don't over-hedge
...
if options_df is not None and not options_df.empty:
    recommendations = recommend_optimal_hedge(
        options_df, current_price, SHARES_HELD, INSURANCE_BUDGET,
        daily_vol, annual_vol, expected_low
    )
else:
    recommendations = []
...
def portfolio_pl_at_price(price: float, positions: List[Dict],
    ...
    for pos in positions:
        exp_date = pos.get('exp_date', today + pd.Timedelta(days=pos.get('days_to_exp', 0)))
...
if recommendations:
    ...
    print(f"{'─'*80}")
    print(f"{'Strike':<8} {'Premium':<10} {'Contracts':<10} {'DTE':<6} {'Cost':<12} {'Efficiency':<12} {'ITM Prob':<10}")
    print(f"{'-'*80}")
    for rec in recommendations:
        print(f"${rec['strike']:>6.2f}  ${rec['premium']:>8.2f}  {rec['contracts']:>9}  "
...
protection_pct = (hedge_benefit / abs(scenario_unhedged["total"])) * 100 if scenario_unhedged["total"] < 0 else 0
...
ax1.plot(df.index, df["SMA20"], label="SMA20", linestyle='--', alpha=0.7, color='#A23B72')
ax1.fill_between(df.index, df["BB_dn"], df["BB_up"], alpha=0.2, color='gray', label="Historical BB")

# Forward projections
ax1.plot(future_dates, proj_sma, linestyle='--', alpha=0.5, color='#A23B72', linewidth=1.5, label="Projected SMA20")
ax1.fill_between(future_dates, proj_bb_dn, proj_bb_up, alpha=0.15, color='orange', label="Projected BB Range")
ax1.plot(future_dates, proj_bb_up, linestyle=':', alpha=0.6, color='orange', linewidth=1)
ax1.plot(future_dates, proj_bb_dn, linestyle=':', alpha=0.6, color='orange', linewidth=1)

# Calculate percentiles for forward projections
prob_expected_low = calculate_price_probability(expected_low, current_price, FORWARD_PROJECTION_DAYS, daily_vol, annual_vol) * 100
prob_expected_high = 100 - calculate_price_probability(expected_high, current_price, FORWARD_PROJECTION_DAYS, daily_vol, annual_vol) * 100
...
percentile_colors = ['#D32F2F', '#FF6F00', '#FFC107', '#8BC34A', '#4CAF50']  # Red to Green gradient
...
for i, percentile in enumerate(percentiles):
    ...
    ax1.plot(future_dates, [price_at_percentile] * len(future_dates),
             linestyle=':', linewidth=1.5, color=percentile_colors[i], alpha=0.7,
             label=f"{percentile}th %ile: ${price_at_percentile:.2f}")

# Mark expected move window with percentiles
ax1.axhline(expected_low, linestyle='--', linewidth=2, color='red', alpha=0.7,
            label=f"Expected Low: ${expected_low:.2f} ({100-prob_expected_low:.0f}th %ile)")
ax1.axhline(expected_high, linestyle='--', linewidth=2, color='green', alpha=0.7,
...
for i, pos in enumerate(puts_to_analyze):
    ...
    ax1.axhline(strike, linestyle='--', linewidth=1.5, alpha=0.7,
                color=strike_colors[i % len(strike_colors)], label=label)
    if exp_date <= future_dates[-1]:
        ax1.axvline(exp_date, linestyle=':', alpha=0.4,
                    color=strike_colors[i % len(strike_colors)], linewidth=1)

ax1.axhline(current_price, color='green', linestyle='-', linewidth=1.5, alpha=0.5,
            label=f"Current: {current_price:.2f}")

ax1.set_title(f"{TICKER} Price History with Forward Projections & {puts_label} Put Strikes",
...
ax2.plot(prices, pl_stock_only, label="Unhedged", linewidth=2.5,
         color='#D62828', linestyle='--', alpha=0.8)
ax2.plot(prices, pl_with_hedge, label=f"Hedged ({puts_label})", linewidth=2.5,
...
ax2.axvline(expected_low, color='red', linewidth=1.5, linestyle='--', alpha=0.7, label="Expected Low")
...
if options_df is not None and not options_df.empty:
    ...
    premiums_available = options_df['lastPrice'].fillna(
    ...
    scatter = ax3.scatter(strikes_available, efficiencies, alpha=0.6, s=50,
                c=premiums_available, cmap='viridis', label="Available Puts")
    ...
    if recommendations:
        ...
        ax3.scatter(rec_strikes, rec_effs, s=200, marker='*',
                   color='red', edgecolors='black', linewidths=2,
                   label="Recommended", zorder=5)

    # Mark expected low
    ax3.axvline(expected_low, linestyle='--', linewidth=2, color='red',
                alpha=0.7, label="Expected Low")
    ax3.axvline(current_price, linestyle=':', linewidth=1, color='green',
    ...
    ax3.text(0.5, 0.5, "No live options data available",
...
probabilities = [calculate_price_probability(p, current_price, max_dte, daily_vol, annual_vol) * 100
                 for p in price_range]

ax4.plot(price_range, probabilities, linewidth=2.5, color='#8B5CF6',
...
ax4.axvline(expected_low, color='red', linewidth=2, linestyle='--', alpha=0.7, label="Expected Low")
ax4.axvspan(proj_bb_dn[-1], proj_bb_up[-1], alpha=0.2, color='orange', label="Projected BB Range")

for pos in puts_to_analyze:
    strike = pos['strike']
    dte = pos.get('dte', max_dte)
    prob = calculate_price_probability(strike, current_price, dte, daily_vol, annual_vol) * 100
    ax4.axvline(strike, linestyle='--', alpha=0.6, linewidth=1)
    ax4.text(strike, prob + 2, f"${strike:.1f}\n({prob:.1f}%)",
             ha='center', fontsize=8, bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))

ax4.set_title(f"Probability Distribution & Put Strike Analysis", fontsize=12, fontweight='bold')
...
for i, pos in enumerate(puts_to_analyze):
    ...
    prob_itm = calculate_price_probability(strike, current_price, dte, daily_vol, annual_vol) * 100
    ...
    rect = Rectangle((x_start, y_start), x_width, bar_height,
...
ax5.text(0.5, 0.95, f"{puts_label} PUT POSITIONS",
...
plt.suptitle(f"{TICKER} AI-Powered Hedge Analysis: Expected Moves vs Available Options",
...
with PdfPages(pdf_filename) as pdf:
    ...
    # Title (APA style)
    ax_summary.text(0.5, 0.96, f"Protective Put Hedge Analysis: {TICKER}",
                     ha='center', fontsize=16, fontweight='bold',
                     transform=fig_summary.transFigure, color='black')
    ax_summary.text(0.5, 0.92, f"Analysis Date: {today.strftime('%B %d, %Y')}",
                     ha='center', fontsize=11, transform=fig_summary.transFigure,
                     color='black')

    # Draw separator line
    line = plt.Line2D([0.1, 0.9], [0.88, 0.88], transform=fig_summary.transFigure,
                      color='black', linewidth=0.5)
    ax_summary.add_line(line)

    # Portfolio Overview Section
    y_pos = 0.82
    ax_summary.text(0.12, y_pos, "Portfolio Overview",
                     fontsize=12, fontweight='bold', transform=fig_summary.transFigure,
                     color='black')
    ...
    for i, (label, value) in enumerate(left_col):
        ax_summary.text(0.12, y_pos, f"{label}:", fontsize=10,
                        transform=fig_summary.transFigure, color='black')
        ax_summary.text(0.35, y_pos, value, fontsize=10, fontweight='bold',
                        transform=fig_summary.transFigure, color='black')
        y_pos -= 0.045

    y_pos = 0.82 - 0.06
    for i, (label, value) in enumerate(right_col):
        ax_summary.text(0.55, y_pos, f"{label}:", fontsize=10,
                        transform=fig_summary.transFigure, color='black')
        ax_summary.text(0.78, y_pos, value, fontsize=10, fontweight='bold',
                        transform=fig_summary.transFigure, color='black')
        y_pos -= 0.045

    # Scenario Analysis Section
    y_pos -= 0.06
    ax_summary.text(0.12, y_pos, "50% Price Decline Scenario Analysis",
                     fontsize=12, fontweight='bold', transform=fig_summary.transFigure,
                     color='black')

    y_pos -= 0.05
    ax_summary.text(0.12, y_pos, "Unhedged Portfolio P/L:", fontsize=10,
                    transform=fig_summary.transFigure, color='black')
    ax_summary.text(0.50, y_pos, f"\\${scenario_unhedged['total']:>12,.0f}",
                    fontsize=11, fontweight='bold', transform=fig_summary.transFigure,
                    color='black', family='monospace')

    y_pos -= 0.04
    ax_summary.text(0.12, y_pos, "Hedged Portfolio P/L:", fontsize=10,
                    transform=fig_summary.transFigure, color='black')
    ax_summary.text(0.50, y_pos, f"\\${scenario_hedged['total']:>12,.0f}",
                    fontsize=11, fontweight='bold', transform=fig_summary.transFigure,
                    color='black', family='monospace')

    y_pos -= 0.04
    ax_summary.text(0.12, y_pos, "Hedge Benefit:", fontsize=10,
                    transform=fig_summary.transFigure, color='black')
    ax_summary.text(0.50, y_pos, f"\\${hedge_benefit:>12,.0f} ({protection_pct:.1f}% protection)",
                    fontsize=11, fontweight='bold', transform=fig_summary.transFigure,
                    color='black', family='monospace')
    ...
    if recommendations:
        ...
        # Title
        ax_rec.text(0.5, 0.93, "Recommended Hedge Positions",
                    ha='center', fontsize=16, fontweight='bold',
                    transform=fig_rec.transFigure, color='black')
        ax_rec.text(0.5, 0.89, f"Optimal Put Options Strategy for {TICKER}",
                    ha='center', fontsize=12, transform=fig_rec.transFigure,
                    color='#555555')

        # Separator line
        line = plt.Line2D([0.1, 0.9], [0.86, 0.86], transform=fig_rec.transFigure,
                          color='black', linewidth=0.5)
        ...
        rec_table = ax_rec.table(cellText=rec_table_data,
                                 colLabels=["Strike", "Premium", "Contracts", "DTE", "Expiration",
                                           "Cost", "ITM Prob", "Efficiency"],
        ...
        # Summary box
        summary_box = FancyBboxPatch((0.1, 0.15), 0.80, 0.15,
                                    boxstyle="round,pad=0.01",
                                    edgecolor='#404040', facecolor='#f0f8ff',
                                    linewidth=1.5, transform=fig_rec.transFigure)
        ax_rec.add_patch(summary_box)

        ax_rec.text(0.5, 0.27, "Strategy Summary",
...
Portfolio Coverage: {(total_contracts*100/SHARES_HELD)*100:.0f}%
        ...
        ax_rec.text(0.5, 0.15, summary_text,
                    fontsize=10, ha='center', transform=fig_rec.transFigure,
                    color='black', family='monospace')

        ax_rec.text(0.5, 0.08, "Note: These recommendations are based on current market conditions and volatility analysis.\\n"
                    "Options strategies involve risk and may not be suitable for all investors.",
                    fontsize=8, ha='center', transform=fig_rec.transFigure,
    ...
    # Title
    ax_vol.text(0.5, 0.90, "Volatility & Price Statistics",
                ha='center', fontsize=16, fontweight='bold',
                transform=fig_vol.transFigure, color='black')
    ax_vol.text(0.5, 0.86, f"Historical Price Analysis & Risk Metrics",
                ha='center', fontsize=12, transform=fig_vol.transFigure,
                color='#555555')

    # Separator line
    line = plt.Line2D([0.15, 0.85], [0.83, 0.83], transform=fig_vol.transFigure,
                      color='black', linewidth=0.5)
    ...
    vol_table_data = [
        ...
        ['Current Price', f'\\${current_price:.2f}'],
        ['52-Week High', f'\\${close.max():.2f}' if len(close) >= 252 else f'\\${close.max():.2f}'],
        ['52-Week Low', f'\\${close.min():.2f}' if len(close) >= 252 else f'\\${close.min():.2f}'],
        ['Price Range', f'\\${close.min():.2f} - \\${close.max():.2f}'],
    ]

    vol_table = ax_vol.table(cellText=vol_table_data,
    ...
    # Title
    ax_risk.text(0.5, 0.90, "Portfolio Risk Assessment",
                 ha='center', fontsize=16, fontweight='bold',
                 transform=fig_risk.transFigure, color='black')
    ax_risk.text(0.5, 0.86, f"Value at Risk & Position Analysis",
                 ha='center', fontsize=12, transform=fig_risk.transFigure,
                 color='#555555')

    # Separator line
    line = plt.Line2D([0.15, 0.85], [0.83, 0.83], transform=fig_risk.transFigure,
                      color='black', linewidth=0.5)
    ...
    risk_table_data = [
        ...
        ['Budget as % of Portfolio', f'{(INSURANCE_BUDGET/portfolio_value)*100:.2f}%'],
    ]

    risk_table = ax_risk.table(cellText=risk_table_data,
    ...
    # Title
    ax_scenario.text(0.5, 0.90, "Downside Scenario Analysis",
                     ha='center', fontsize=16, fontweight='bold',
                     transform=fig_scenario.transFigure, color='black')
    ax_scenario.text(0.5, 0.86, f"Hedge Performance Across Multiple Price Declines",
                     ha='center', fontsize=12, transform=fig_scenario.transFigure,
                     color='#555555')

    # Separator line
    line = plt.Line2D([0.15, 0.85], [0.83, 0.83], transform=fig_scenario.transFigure,
                      color='black', linewidth=0.5)
    ...
    for scenario_name, price_mult in scenarios:
        ...
        protection = (benefit / abs(unhedged_pl["total"])) * 100 if unhedged_pl["total"] < 0 else 0
    ...
    scenario_table = ax_scenario.table(cellText=scenario_table_data,
                                     colLabels=['Scenario', 'Price', 'Unhedged P/L',
                                               'Hedged P/L', 'Benefit', 'Protection'],
                                     cellLoc='center', loc='center',
    ...
    ax_price.plot(df.index, df["SMA20"], label="SMA20", linestyle='--', alpha=0.7, color='#A23B72')
    ax_price.fill_between(df.index, df["BB_dn"], df["BB_up"], alpha=0.2, color='gray', label="Historical BB")

    ax_price.plot(future_dates, proj_sma, linestyle='--', alpha=0.5, color='#A23B72', linewidth=1.5, label="Projected SMA20")
    ax_price.fill_between(future_dates, proj_bb_dn, proj_bb_up, alpha=0.15, color='orange', label="Projected BB Range")
    ax_price.plot(future_dates, proj_bb_up, linestyle=':', alpha=0.6, color='orange', linewidth=1)
    ax_price.plot(future_dates, proj_bb_dn, linestyle=':', alpha=0.6, color='orange', linewidth=1)
    ...
    for i, (percentile, price_at_percentile) in enumerate(percentile_prices):
        ax_price.plot(future_dates, [price_at_percentile] * len(future_dates),
                     linestyle=':', linewidth=1.5, color=percentile_colors[i], alpha=0.7,
                     label=f"{percentile}th %ile: ${price_at_percentile:.2f}")

    ax_price.axhline(expected_low, linestyle='--', linewidth=2, color='red', alpha=0.7, label=f"Expected Low: ${expected_low:.2f}")
    ax_price.axhline(expected_high, linestyle='--', linewidth=2, color='green', alpha=0.7, label=f"Expected High: ${expected_high:.2f}")
    ...
    for i, pos in enumerate(puts_to_analyze):
        ...
        exp_date = pos.get('exp_date', today + pd.Timedelta(days=pos.get('dte', 0)))
        label = f"{pos.get('dte', 'N/A')}d Put @ ${strike:.1f}"
        ax_price.axhline(strike, linestyle='--', linewidth=1.5, alpha=0.7,
                        color=strike_colors[i % len(strike_colors)], label=label)
        if exp_date <= future_dates[-1]:
            ax_price.axvline(exp_date, linestyle=':', alpha=0.4,
                            color=strike_colors[i % len(strike_colors)], linewidth=1)

    ax_price.axhline(current_price, color='green', linestyle='-', linewidth=1.5, alpha=0.5,
                    label=f"Current: {current_price:.2f}")

    ax_price.set_title(f"Figure 1: {TICKER} Price History with Forward Projections & {puts_label} Put Strikes",
    ...
    ax_pl.plot(prices, pl_stock_only, label="Unhedged", linewidth=2.5,
             color='#D62828', linestyle='--', alpha=0.8)
    ax_pl.plot(prices, pl_with_hedge, label=f"Hedged ({puts_label})", linewidth=2.5,
             color='#06A77D', linestyle='-')

    ax_pl.axhline(0, color='black', linewidth=1, linestyle='-', alpha=0.5)
    ax_pl.axvline(current_price, color='green', linewidth=1, linestyle=':', alpha=0.5, label="Current Price")
    ax_pl.axvline(expected_low, color='red', linewidth=1.5, linestyle='--', alpha=0.7, label="Expected Low")

    ax_pl.set_title("Figure 2: P/L Comparison: Hedged vs Unhedged", fontsize=12, fontweight='bold')
    ...
    if options_df is not None and not options_df.empty:
        ...
        scatter = ax_eff.scatter(strikes_available, efficiencies, alpha=0.6, s=50,
                    c=premiums_available, cmap='viridis', label="Available Puts")
        ...
        if recommendations:
            rec_strikes = [r['strike'] for r in recommendations]
            rec_effs = [r['efficiency_score'] for r in recommendations]
            ax_eff.scatter(rec_strikes, rec_effs, s=200, marker='*',
                       color='red', edgecolors='black', linewidths=2,
                       label="Recommended", zorder=5)

        ax_eff.axvline(expected_low, linestyle='--', linewidth=2, color='red',
                    alpha=0.7, label="Expected Low")
        ax_eff.axvline(current_price, linestyle=':', linewidth=1, color='green',
                    alpha=0.5, label="Current Price")

        ax_eff.set_title("Figure 4: Put Options Efficiency Analysis", fontsize=12, fontweight='bold')
    ...
    ax_prob.plot(price_range, probabilities, linewidth=2.5, color='#8B5CF6',
             label=f"Probability (next {max_dte} days)")
    ax_prob.fill_between(price_range, 0, probabilities, alpha=0.3, color='#8B5CF6')

    ax_prob.axvline(current_price, color='green', linewidth=1.5, linestyle='-', alpha=0.7)
    ax_prob.axvline(expected_low, color='red', linewidth=2, linestyle='--', alpha=0.7, label="Expected Low")
    ax_prob.axvspan(proj_bb_dn[-1], proj_bb_up[-1], alpha=0.2, color='orange', label="Projected BB Range")
    ...
    for pos in puts_to_analyze:
        ...
        ax_prob.text(strike, prob + 2, f"${strike:.1f}\n({prob:.1f}%)",
                 ha='center', fontsize=8, bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))

    ax_prob.set_title("Figure 5: Probability Distribution & Put Strike Analysis", fontsize=12, fontweight='bold')
    ...
    # Header
    ax_pos.text(0.5, 0.96, f"{puts_label} Hedge Positions: Detailed Analysis",
             fontsize=16, fontweight='bold', ha='center', transform=fig_pos.transFigure,
             color='black')
    ax_pos.text(0.5, 0.92, f"Position-by-Position Breakdown & Performance Metrics",
             fontsize=11, ha='center', transform=fig_pos.transFigure,
             color='#555555')

    # Separator line
    line = plt.Line2D([0.05, 0.95], [0.89, 0.89], transform=fig_pos.transFigure,
                      color='black', linewidth=0.5)
    ...
    # Add total rows
    total_cost = sum(p.get('premium', 0) * 100 * p.get('contracts', 0) for p in puts_to_analyze)
    total_value_50 = sum(max(p.get('strike', 0) - scenario_price_50_down, 0) * 100 * p.get('contracts', 0)
                         for p in puts_to_analyze) - total_cost
    total_ratio = f"{(total_value_50/total_cost)*100:.1f}" if total_cost > 0 else "N/A"
    ...
    ax_pos.text(0.5, 0.87, "Position Details",
                fontsize=12, fontweight='bold', ha='center',
                transform=fig_pos.transFigure, color='#404040')

    # Create Table 1: Position Details
    table1 = ax_pos.table(cellText=table1_data, colLabels=headers1,
                          cellLoc='center', loc='center',
    ...
    ax_pos.text(0.5, 0.58, "Financial Metrics",
                fontsize=12, fontweight='bold', ha='center',
                transform=fig_pos.transFigure, color='#404040')


    # Create Table 2: Financial Metrics
    table2 = ax_pos.table(cellText=table2_data, colLabels=headers2,
                          cellLoc='center', loc='center',
    ...
    # Draw section box
    summary_box = FancyBboxPatch((0.10, 0.12), 0.80, 0.15,
                                boxstyle="round,pad=0.01",
                                edgecolor='#404040', facecolor='#f5f5f5',
                                linewidth=1.5, transform=fig_pos.transFigure)
    ax_pos.add_patch(summary_box)

    ax_pos.text(0.5, 0.25, "Portfolio Summary Statistics",
                fontsize=12, fontweight='bold', ha='center',
                transform=fig_pos.transFigure, color='black')
    ...
    ax_pos.text(col1_x, summary_y, "Total Premium:",
                fontsize=9, ha='left', transform=fig_pos.transFigure, color='#333333')
    ax_pos.text(col1_x, summary_y - 0.03, f"\\${scenario_hedged['premium_paid']:,.0f}",
                fontsize=11, fontweight='bold', ha='left',
                transform=fig_pos.transFigure, color='black', family='monospace')

    ax_pos.text(col2_x, summary_y, "Hedge Benefit (50% drop):",
                fontsize=9, ha='left', transform=fig_pos.transFigure, color='#333333')
    ax_pos.text(col2_x, summary_y - 0.03, f"\\${hedge_benefit:,.0f}",
                fontsize=11, fontweight='bold', ha='left',
                transform=fig_pos.transFigure, color='#006600', family='monospace')

    ax_pos.text(col3_x, summary_y, "Protection Level:",
                fontsize=9, ha='left', transform=fig_pos.transFigure, color='#333333')
    ax_pos.text(col3_x, summary_y - 0.03, f"{protection_pct:.1f}%",
                fontsize=11, fontweight='bold', ha='left',
                transform=fig_pos.transFigure, color='#006600', family='monospace')

    ax_pos.text(col1_x, 0.14, f"Annual Volatility: {annual_vol*100:.1f}% | "
                f"Analysis based on {LOOKBACK_DAYS}-day historical data",
                fontsize=8, ha='left', transform=fig_pos.transFigure,
                color='#666666', style='italic')
...
if recommendations:
    print("💡 RECOMMENDATION: Consider implementing the recommended positions for optimal cost efficiency.\n")
